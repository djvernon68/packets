...
from packets.core.pcap import PCAPReader, PCAPWriter, \
    ...
    validate_bpf_filter, _build_netflow_replay_frame
...
class TestPackets(unittest.TestCase):
    ...
    def test_validate_bpf_filter_accepts_valid(self):
        # A well-formed expression compiles: ok is True and there is no
        # error text to report.
        ok, err = validate_bpf_filter('ip and udp dst port 2055')
        self.assertTrue(ok)
        self.assertIsNone(err)

    def test_validate_bpf_filter_accepts_empty(self):
        # An empty filter is legal in libpcap - it matches every packet -
        # so the validator must not treat it as an error.
        ok, err = validate_bpf_filter('')
        self.assertTrue(ok)
        self.assertIsNone(err)

    def test_validate_bpf_filter_rejects_invalid(self):
        # A bad expression compiles to a failure: ok is False and err carries
        # libpcap's own description (pcap_geterr text) rather than a bare code.
        ok, err = validate_bpf_filter('this is not a bpf')
        self.assertFalse(ok)
        self.assertIsInstance(err, str)
        self.assertTrue(err)

    def test_validate_bpf_filter_is_side_effect_free(self):
        # Validation needs no device or capture file and installs nothing.
        # Repeated calls (including a failing one) must be safe, and a real
        # reader opened afterwards must still hand back every packet because
        # no filter was left installed on anything.
        import os
        for _ in range(3):
            self.assertEqual(validate_bpf_filter('tcp port 80')[0], True)
            self.assertEqual(validate_bpf_filter('nonsense !!')[0], False)
        cap = os.path.join(os.path.dirname(__file__), 'igmp_v2.pcap')
        self.assertGreater(len(list(PCAPReader(filename=cap))), 0)

    def test_validate_bpf_filter_matches_add_bpf_filter(self):
        # The validator and add_bpf_filter() must agree on what compiles:
        # what validate_bpf_filter accepts, add_bpf_filter installs without
        # raising; what it rejects, add_bpf_filter raises on.
        import os
        cap = os.path.join(os.path.dirname(__file__), 'igmp_v2.pcap')

        good = 'ip and udp dst port 2055'
        self.assertTrue(validate_bpf_filter(good)[0])
        rdr = PCAPReader(filename=cap)
        self.assertEqual(rdr.add_bpf_filter(good), 0)
        rdr.close()

        bad = 'this is not a bpf'
        self.assertFalse(validate_bpf_filter(bad)[0])
        rdr = PCAPReader(filename=cap)
        self.assertRaises(Exception, rdr.add_bpf_filter, bad)
        rdr.close()

    def test_validate_bpf_filter_linktype_argument(self):
        # The link type the filter is compiled against is selectable. A
        # link-layer primitive that is meaningful for Ethernet should still
        # validate when Ethernet is named explicitly.
        ok, err = validate_bpf_filter('ether host 00:11:22:33:44:55',
                                      linktype=_pcap.PCAP_CONST().ETH_EN10MB)
        self.assertTrue(ok)
        self.assertIsNone(err)
